...
def generarMenu(usuario):
    ...
    with st.sidebar: # Creamos una barra lateral para el menú
        ...
        st.subheader("Tipo de documentación") # Subtítulo para los tableros
        # Mostramos los enlaces a las páginas según el rol del usuario
        if rol in ['profesores','directivos']:
            st.page_link("pages/gestor.py", label="Profesores", icon=":material/sell:") # Enlace a la página de ventas        
        if rol in ['profesores','monitores','directivos']:
            st.page_link("pages/page1.py", label="Compras", icon=":material/shopping_cart:") # Enlace a la página de compras        
